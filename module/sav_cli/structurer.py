...
def structure_player(dir_path: str, data_source: None | Any = None) -> list[Any]:
    """NOTE: Requires documentation from the original owner."""
    ...
    for pal in pals:
        for player in unique_players:
            if player["player_uid"] == pal["owner"]:
                ...
                break
    sorted_players: list[Any] = sorted(players, key=lambda p: p["level"], reverse=True)
...
def structure_guild(filetime: int = -1):
    """NOTE: Requires documentation from the original owner."""
>>>>>>> upstream/main:module/structurer.py
    ...
    guilds_generator = (Guild(g, real_date_time_ticks, filetime).to_dict() for g in groups)
